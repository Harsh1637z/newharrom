...
// Handle form submissions for adding/editing/deleting classes
    ...
    if (isset($_POST['action'])) {
        ...
        if ($action === 'add') {
            ...
            if (empty($name) || empty($section)) {
                ...
                $query = "INSERT INTO classes (name, section) VALUES (?, ?)";
                $stmt = mysqli_prepare($db_conn, $query);
                if ($stmt) {
                    mysqli_stmt_bind_param($stmt, "ss", $name, $section);
            ...
            if ($class_id <= 0) {
                ...
                if (empty($name) || empty($section)) {
                    ...
                    $query = "UPDATE classes SET name = ?, section = ? WHERE id = ?";
                    $stmt = mysqli_prepare($db_conn, $query);
                    if ($stmt) {
                        mysqli_stmt_bind_param($stmt, "ssi", $name, $section, $class_id);
                        if (mysqli_stmt_execute($stmt)) {
...
// Fetch all classes
$query = "SELECT id, name, section, created_at FROM classes ORDER BY created_at DESC";
...
if ($result) {
    ...
    $error = "Failed to fetch classes: " . mysqli_error($db_conn);
}
?>
...
<head>
    ...
    <title>Manage Classes - SMS</title>
            ...
            <nav class="mt-2">
                <ul class="nav nav-pills nav-sidebar flex-column nav-legacy" data-widget="treeview" role="menu" data-accordion="true">
                    ...
                    <li class="nav-item has-treeview menu-open">
                        ...
                        <ul class="nav nav-treeview">
                            <li class="nav-item">
                                <a href="/hrproject/admin/manage-classes.php" class="nav-link active">
                                    <i class="far fa-circle nav-icon"></i>
                                    <p>Classes</p>
                                </a>
                            </li>
                    ...
                    <div class="card-body">
                        <form method="POST">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="section" name="section" required placeholder="e.g., A, B, C">
                            </div>
                            ...
                            <tbody>
                                <?php foreach ($classes as $class): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <button class="btn btn-sm btn-primary" data-toggle="modal" data-target="#editModal<?php echo $class['id']; ?>">Edit</button>
                                    ...
                                    <!-- Edit Class Modal -->
                                    <div class="modal fade" id="editModal<?php echo $class['id']; ?>" tabindex="-1" role="dialog">
                                            ...
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form method="POST">
                                                        ...
                                                        <input type="hidden" name="class_id" value="<?php echo $class['id']; ?>">
                                                        <div class="form-group">
                                                            <label for="name<?php echo $class['id']; ?>">Class Name</label>
                                                            <input type="text" class="form-control" id="name<?php echo $class['id']; ?>" name="name" value="<?php echo htmlspecialchars($class['name']); ?>" required>
                                                        </div>
                                                        <div class="form-group">
                                                            <label for="section<?php echo $class['id']; ?>">Section</label>
                                                            <input type="text" class="form-control" id="section<?php echo $class['id']; ?>" name="section" value="<?php echo htmlspecialchars($class['section']); ?>" required placeholder="e.g., A, B, C">
                                                        </div>
